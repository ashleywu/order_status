...
**精油规格（2026；后台字段，M3 `MaterialDto` 第一版不含）**：

| API 名 | 类型 |
|--------|------|
| `aroma_family` | single select |
| `fragrance_note` | text（如 `top`、`(base, fixatives)`） |
| `inci_name` | text |
| `botanical_name` | text |
| `extraction_method` | text |
| `plant_part` | text |
| `origin` | text |
| `cultivation` | single select |
| `extraction_year` | number |
| `safety_note` | text |
| `bottling_date` | date |
| `inventory` | number |

手工建列：[airtable/ADD-INGREDIENT-FIELDS.txt](airtable/ADD-INGREDIENT-FIELDS.txt)
